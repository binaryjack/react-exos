...
const Store: IStore = {
  subscribe: (callback: () => void) => {
    ...
    /** clean (comme dans un useEffect on nettoie lorsqu'on quitte) */
...
const useMyStore = () => {
  const state = useSyncExternalStore<IProductState>(
    Store.subscribe,
    Store.getSnapshot,
    () => ({ ...INITIAL_STATE })
  );

  const init = useCallback((payload: IProduct[]) => {
    Store.init(payload);
  }, []);

  const update = useCallback((payload: { id: number; name: string }) => {
    Store.update(payload);
  }, []);

  const select = useCallback((payload: IProduct) => {
    Store.select(payload);
  }, []);
...
export const ProductsUseSyncExternalStore = () => {
  const { init, select, state, update } = useMyStore();
